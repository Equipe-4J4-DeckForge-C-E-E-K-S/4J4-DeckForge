Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Pro/c7d22f1ac7b86df49963d49c28de7ad1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float FindTPSSystem(inout float3 orificePosition, inout float3 orificeNormal, inout float penetratorType, float penetratorLength) {
				...
				[loop]for (int i = 0; i < 4; i++) {
					if (abs(lightRanges[i] - ID_RING) < 0.001 || abs(lightRanges[i] - ID_HOLE) < 0.001) {
						if (dot(float4(0,-0.9998,0,0), lightPositions[i]) > 0) {
							float3 foundNormal = -float4(0,-0.9998,0,0);
							...
							if (dot(foundNormal, float4(0,-0.9998,0,0)) < 0 && length(lightPositions[i]) < closestOrfDist) {
				...
				if (length(orificeNormal) == 0) {
					orificeNormal = lerp(normalize(-orificePosition), -float4(0,-0.9998,0,0), max(dot(normalize(orificePosition), float4(0,-0.9998,0,0)), 0.5));
				}
				#ifdef TPS_IsSkinnedMesh
			...
			void ApplyTPSPenetrator(inout float4 vertex, inout float3 normal, inout float4 vertexColor, uint vertexId, float2 uv) {
				...
				float penetratorLength = (0.3289073 /*_TPS_PenetratorLength*/);
				float2 shrinkage = float2((1.0 /*_TPS_IdleSkrinkWidth*/), (1.0 /*_TPS_IdleSkrinkLength*/));
				float3 targetPosition = float4(0,-0.9998,0,0) * penetratorLength;
				float3 targetNormal = -float4(0,-0.9998,0,0);
				...
				penetratorVertex = penetratorVertex * float4(0.03,0.03000001,0.03000001,0);
				#endif
				float penZ = VectorLengthIntoDirection(penetratorVertex, float4(0,-0.9998,0,0));
				vertexColor.a = penZ / penetratorLength;
				float3 bezier0 = float4(0,-0.9998,0,0) * (0.0 /*_TPS_BezierStart*/);
				float3 bezier0Out = bezier0 + float4(0,-0.9998,0,0) * bezierSmoothness;
				...
				float3 penDirectionRight = normalize(cross(float4(0,0,1,0), normalize(bezier1 - bezier0)));
				...
				if (bezierStrength > 0) {
					...
					float squeeze = lerp(0, (0.442 /*_TPS_Squeeze*/), saturate(penetrationDepth * 20));
					sizeChange *= lerp(1, 1 - squeeze, saturate(1 - abs(bezierStrengthUncapped - 1) / (0.158 /*_TPS_SqueezeDistance*/)) * tpsSmoothStart);
					sizeChange *= lerp(1, 1 + (1.237 /*_TPS_Buldge*/), buldgeLerp * tpsSmoothStart2);
					...
					float y = VectorLengthIntoDirection(penetratorVertex, float4(0,0,1,0));
					float3 normalX = VectorLengthIntoDirection(normal, float4(1,0,0,0));
					float3 normalY = VectorLengthIntoDirection(normal, float4(0,0,1,0));
					float3 normalZ = VectorLengthIntoDirection(normal, float4(0,-0.9998,0,0));
					float3 vertexZ = float4(0,-0.9998,0,0) * VectorLengthIntoDirection(vertex, float4(0,-0.9998,0,0));
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				#ifdef TPS_Penetrator
				if ((0.0 /*_TPS_BufferedDepthGlobalMaskIndex*/) > 0)
				{
					applyToGlobalMask(poiMods, (0.0 /*_TPS_BufferedDepthGlobalMaskIndex*/)-1, (0.0 /*_TPS_BufferedDepthGlobalMaskBlendType*/), TPSBufferedDepth(poiMesh.localPos, poiMesh.vertexColor));
				}
				#endif
