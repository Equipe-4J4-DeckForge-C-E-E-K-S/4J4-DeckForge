Shader "Hidden/Locked/.poiyomi/Old Versions/8.2/Poiyomi Pro/62bd3e172e0c0694182f74da70ac82c1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			#define ID_ENHANCED (((0 /*_TPS_Channel*/) - 1) * ID_ENHANCED_STEPS + ID_ENHANCED_MIN)
			#define ID_HOLE ((0 /*_TPS_Channel*/) == -1 ? ID_DPS_CHANNEL_HOLE : ((0 /*_TPS_Channel*/) > 0 ? ID_ENHANCED : ID_DEFAULT_HOLE))
			#define ID_RING ((0 /*_TPS_Channel*/) == -1 ? ID_DPS_CHANNEL_RING : ((0 /*_TPS_Channel*/) > 0 ? ID_ENHANCED : ID_DEFAULT_RING))
			#define ID_NORM ((0 /*_TPS_Channel*/) == -1 ? ID_DPS_CHANNEL_NORM : ((0 /*_TPS_Channel*/) > 0 ? ID_ENHANCED : ID_DEFAULT_NORM))
			...
			#define BuldgeStrength(type) (0.0 /*_TPS_Buldge*/) * (type == ORIFICE_TYPE_HOLE ? float4(1,1,1,1)[0] : float4(1,1,1,1)[1])
			#define SqueezeStrength(type) (0.0 /*_TPS_Squeeze*/) * (type == ORIFICE_TYPE_HOLE ? float4(1,1,1,1)[0] : float4(1,1,1,1)[1])
			...
			void ApplyTPSPenetrator(inout float4 vertex, inout float3 normal, inout float4 vertexColor, uint vertexId, float2 uv) {
				...
				float penetratorLength = (0.1315143 /*_TPS_PenetratorLength*/);
				...
				float bezierSmoothness = (0.4 /*_TPS_BezierSmoothness*/) * penetratorLength;
				...
				if (bezierStrength > 0) {
					float sizeChange = 1;
					if ((0.0 /*_TPS_PumpingStrength*/) > 0) {
						sizeChange *= lerp(1 - (0.0 /*_TPS_PumpingStrength*/) * tpsSmoothStart2, 1 + (0.0 /*_TPS_PumpingStrength*/) * tpsSmoothStart2, abs(sin(_Time.y * -(0.0 /*_TPS_PumpingSpeed*/) + bezierStrengthUncapped / (0.3 /*_TPS_PumpingWidth*/))));
					}
					float buldgeLerp = 0;
					if (bezierStrength < 1 - (0.05 /*_TPS_BuldgeFalloffDistance*/))
					buldgeLerp = saturate((bezierStrength - 1 + (0.2 /*_TPS_BuldgeDistance*/) + (0.05 /*_TPS_BuldgeFalloffDistance*/)) / (0.2 /*_TPS_BuldgeDistance*/));
					else
					buldgeLerp = saturate((-bezierStrength + 1) / (0.05 /*_TPS_BuldgeFalloffDistance*/));
					float squeeze = lerp(0, SqueezeStrength(orificeType), saturate(penetrationDepth * 20));
					sizeChange *= lerp(1, 1 - squeeze, saturate(1 - abs(bezierStrengthUncapped - 1) / (0.01 /*_TPS_SqueezeDistance*/)) * tpsSmoothStart);
			...
			{
				float smoothness = (0.421 /*_MochieRoughnessMultiplier*/);
