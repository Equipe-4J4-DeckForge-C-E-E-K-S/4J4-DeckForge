Shader "Hidden/Locked/.poiyomi/Old Versions/8.2/Poiyomi Pro/8b93b8a55f9aa1249939c948ef1d0d0e"
{
	Properties
	{
 ...
 #define TPS_IsSkinnedMesh 
 #define TPS_Penetrator 
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 _TPS_SqueezeTypeMultiplier;
			float _TPS_Buldge;
			float _TPS_BuldgeDistance;
			float _TPS_BuldgeFalloffDistance;
			float4 _TPS_BuldgeTypeMultiplier;
			...
			#endif
			#define ID_DEFAULT_HOLE 0.41 * 500
			#define ID_DEFAULT_RING 0.42 * 500
			#define ID_DEFAULT_NORM 0.45 * 500
			#define ID_DPS_CHANNEL_HOLE 0.43 * 500
			#define ID_DPS_CHANNEL_RING 0.44 * 500
			#define ID_DPS_CHANNEL_NORM 0.46 * 500
			#define ID_ENHANCED_MIN 0.452 * 500
			#define ID_ENHANCED_STEPS 0.002 * 500
			#define TPS_ALPHA_ENHANCED_MASK 192
			#define TPS_ALPHA_ENHANCED_CHECK 64
			#define TPS_ALPHA_TYPE_MASK 48
			#define ORIFICE_TYPE_HOLE 0
			#define ORIFICE_TYPE_RING 1
			#define ORIFICE_TYPE_NORMAL 2
			#endif
			struct appdata
			{
			...
			#endif
			#define RoundLightRange(range) (round(range / 0.002))
			#define BitValueCheck(value,mask,bits) ((value & mask) == bits)
			#define ID_ENHANCED (((14 /*_TPS_Channel*/) - 1) * ID_ENHANCED_STEPS + ID_ENHANCED_MIN)
			#define ID_HOLE ((14 /*_TPS_Channel*/) == -1 ? ID_DPS_CHANNEL_HOLE : ((14 /*_TPS_Channel*/) > 0 ? ID_ENHANCED : ID_DEFAULT_HOLE))
			#define ID_RING ((14 /*_TPS_Channel*/) == -1 ? ID_DPS_CHANNEL_RING : ((14 /*_TPS_Channel*/) > 0 ? ID_ENHANCED : ID_DEFAULT_RING))
			#define ID_NORM ((14 /*_TPS_Channel*/) == -1 ? ID_DPS_CHANNEL_NORM : ((14 /*_TPS_Channel*/) > 0 ? ID_ENHANCED : ID_DEFAULT_NORM))
			#define IsLightEnhancedAlpha(alpha) (BitValueCheck(alpha,TPS_ALPHA_ENHANCED_MASK,TPS_ALPHA_ENHANCED_CHECK))
			#define VectorLengthIntoDirection(v,d) dot(v,d)
			#define BuldgeStrength(type) (1.237 /*_TPS_Buldge*/) * (type == ORIFICE_TYPE_HOLE ? float4(1,1,1,1)[0] : float4(1,1,1,1)[1])
			#define SqueezeStrength(type) (0.442 /*_TPS_Squeeze*/) * (type == ORIFICE_TYPE_HOLE ? float4(1,1,1,1)[0] : float4(1,1,1,1)[1])
			float FindTPSSystem(inout float3 orificePosition, inout float3 orificeNormal, inout float penetratorType, float penetratorLength) {
				...
				[loop] for (int f = 0; f < 4; f++) {
					...
					lightRanges[f] = RoundLightRange(LightRange(f));
					lightAlpha[f] = int((unity_LightColor[f].a) * 255);
					tpsType[f] = -1;
					isEnhanced[f] = IsLightEnhancedAlpha(lightAlpha[f]);
					if(lightRanges[f] == ID_HOLE || lightRanges[f] == ID_RING || lightRanges[f] == ID_NORM)
					{
						if(isEnhanced[f])
						tpsType[f] = (lightAlpha[f] & TPS_ALPHA_TYPE_MASK) >> 4;
						else
						tpsType[f] = lightRanges[f] == ID_HOLE ? ORIFICE_TYPE_HOLE : (lightRanges[f] == ID_RING ? ORIFICE_TYPE_RING : ORIFICE_TYPE_NORMAL);
					}
				}
				float closestOrfDist = 100000000;
				[loop]for (int i = 0; i < 4; i++) {
					if(tpsType[i] > -1 && tpsType[i] != ORIFICE_TYPE_NORMAL)
					{
						if (dot(float4(0,0,1,0), lightPositions[i]) > 0) {
							float3 foundNormal = -float4(0,0,1,0);
							bool hasNormal = false;
							[loop] for (int n = 0; n < 4; n++) {
								if (tpsType[n] == ORIFICE_TYPE_NORMAL && distance(lightPositionsWorld[n] , lightPositionsWorld[i]) < 0.05) {
									foundNormal = normalize(lightPositions[n] - lightPositions[i]);
									hasNormal = true;
								}
							}
							if (dot(foundNormal, float4(0,0,1,0)) < 0 && length(lightPositions[i]) < closestOrfDist) {
								closestOrfDist = length(lightPositions[i]);
								orificePosition = lightPositions[i];
								penetratorType = tpsType[i];
				...
				if (length(orificeNormal) == 0) {
					orificeNormal = lerp(normalize(-orificePosition), -float4(0,0,1,0), max(dot(normalize(orificePosition), float4(0,0,1,0)), 0.5));
				}
				#ifdef TPS_IsSkinnedMesh
				return (closestOrfDist) < (((1.0 /*_TPS_SmoothStart*/)+1) * penetratorLength);
				#else
				return (closestOrfDist) < (min((1.0 /*_TPS_SmoothStart*/)+1,1.5) * penetratorLength);
				#endif
			}
			...
			void ApplyTPSPenetrator(inout float4 vertex, inout float3 normal, inout float4 vertexColor, uint vertexId, float2 uv) {
				...
				float penetratorLength = (0.3290389 /*_TPS_PenetratorLength*/);
				float2 shrinkage = float2((1.0 /*_TPS_IdleSkrinkWidth*/), (1.0 /*_TPS_IdleSkrinkLength*/));
				float3 targetPosition = float4(0,0,1,0) * penetratorLength;
				float3 targetNormal = -float4(0,0,1,0);
				float tpsSmoothStart = 0;
				float tpsSmoothStart2 = 0;
				float bezierSmoothness = (0.17 /*_TPS_BezierSmoothness*/) * penetratorLength;
				float penetrationDepth = 0;
				 if (FindTPSSystem(orificePosition, orificeNormal, orificeType, penetratorLength) )
				{
					#ifdef TPS_IsSkinnedMesh
					float smoothStartStart = (1.0 /*_TPS_SmoothStart*/);
					#else
					float smoothStartStart = min((1.0 /*_TPS_SmoothStart*/),0.5);
					#endif
					...
					shrinkage = lerp(float2((1.0 /*_TPS_IdleSkrinkWidth*/), (1.0 /*_TPS_IdleSkrinkLength*/)), float2(1, 1), saturate(tpsSmoothStart * 2));
				}
				...
				penetratorVertex = penetratorVertex * float4(1,1,1,0);
				#endif
				float penZ = VectorLengthIntoDirection(penetratorVertex, float4(0,0,1,0));
				vertexColor.a = penZ / penetratorLength;
				float3 bezier0 = float4(0,0,1,0) * (0.0 /*_TPS_BezierStart*/);
				float3 bezier0Out = bezier0 + float4(0,0,1,0) * bezierSmoothness;
				float3 bezier1 = targetPosition;
				float3 bezier1In = bezier1 + targetNormal * bezierSmoothness;
				float bezierStrengthUncapped = ((penZ - (0.0 /*_TPS_BezierStart*/)) / distance(bezier0, bezier1));
				...
				if (orificeType == ORIFICE_TYPE_RING && bezierStrength == 1 && tpsSmoothStart == 1) {
					bezierPoint = orificePosition + (penZ - (0.0 /*_TPS_BezierStart*/) - length(orificePosition.xyz)) * -orificeNormal;
					bezierPoint2 = bezierPoint - orificeNormal;
				}
				float3 penDirectionRight = normalize(cross(float4(0,1,0,0), normalize(bezier1 - bezier0)));
				...
				#endif
				if((1.0 /*_TPS_AnimatedToggle*/) == 0){
					bezierStrength = 0;
				}
				if (bezierStrength > 0) {
					float sizeChange = 1;
					if ((0.054 /*_TPS_PumpingStrength*/) > 0) {
						sizeChange *= lerp(1 - (0.054 /*_TPS_PumpingStrength*/) * tpsSmoothStart2, 1 + (0.054 /*_TPS_PumpingStrength*/) * tpsSmoothStart2, abs(sin(_Time.y * -(3.92 /*_TPS_PumpingSpeed*/) + bezierStrengthUncapped / (0.201 /*_TPS_PumpingWidth*/))));
					}
					float buldgeLerp = 0;
					if (bezierStrength < 1 - (0.02 /*_TPS_BuldgeFalloffDistance*/))
					buldgeLerp = saturate((bezierStrength - 1 + (0.402 /*_TPS_BuldgeDistance*/) + (0.02 /*_TPS_BuldgeFalloffDistance*/)) / (0.402 /*_TPS_BuldgeDistance*/));
					else
					buldgeLerp = saturate((-bezierStrength + 1) / (0.02 /*_TPS_BuldgeFalloffDistance*/));
					float squeeze = lerp(0, SqueezeStrength(orificeType), saturate(penetrationDepth * 20));
					sizeChange *= lerp(1, 1 - squeeze, saturate(1 - abs(bezierStrengthUncapped - 1) / (0.158 /*_TPS_SqueezeDistance*/)) * tpsSmoothStart);
					sizeChange *= lerp(1, 1 + BuldgeStrength(orificeType), buldgeLerp * tpsSmoothStart2);
					shrinkage = lerp(1, shrinkage, saturate((penZ - (0.0 /*_TPS_BezierStart*/)) * 20));
					float x = VectorLengthIntoDirection(penetratorVertex, float4(1,0,0,0));
					float y = VectorLengthIntoDirection(penetratorVertex, float4(0,1,0,0));
					float3 normalX = VectorLengthIntoDirection(normal, float4(1,0,0,0));
					float3 normalY = VectorLengthIntoDirection(normal, float4(0,1,0,0));
					float3 normalZ = VectorLengthIntoDirection(normal, float4(0,0,1,0));
					float3 vertexZ = float4(0,0,1,0) * VectorLengthIntoDirection(vertex, float4(0,0,1,0));
				...
				if ((bezierStrengthUncapped * tpsSmoothStart) > 1 && orificeType == ORIFICE_TYPE_HOLE) //Disc fix
				vertex.xyz = lerp(vertex.xyz, bezierPoint, tpsSmoothStart);
			}
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalRotation = (83.2 /*_DecalRotation*/);
				Decal0.m_DecalScale = float4(0.3,0.2,0,0);
				Decal0.m_DecalSideOffset = float4(0,0,0,0);
				Decal0.m_DecalPosition = float4(0.18,0.44,0,0);
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				half3 glitterColor = poiThemeColor(poiMods, float4(1,0.5377358,0.7500139,1), (0.0 /*_GlitterColorThemeIndex*/));
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.213173,0,0.6396053,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.213173,0,0.6396053,1).a;
